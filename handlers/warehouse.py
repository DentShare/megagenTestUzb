...
async def is_warehouse(user_id: int, session: AsyncSession) -> bool:
    """Проверка прав склада с использованием кеша."""
    ...
    return user and user.role == UserRole.WAREHOUSE and user.is_active
...
async def warehouse_order_detail(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if order.items:
        for idx, item in enumerate(order.items, 1):
            text += f"{idx}. {escape_markdown(item.item_name)} — {item.quantity} шт.\n"
    else:
    ...
    await safe_edit_text(
        callback.message,
        text,
        reply_markup=get_warehouse_order_detail_kb(order.id, order.status.value),
    )
    await callback.answer()
...
async def take_order(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if order and order.status == OrderStatus.NEW:
        ...
        await callback.message.edit_reply_markup(reply_markup=get_warehouse_order_detail_kb(order.id, "assembly"))
...
async def ready_order(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    # Данные для QR1 — товары в заказе
    items_data = [
        {"name": item.item_name, "qty": item.quantity}
    ...
    if order:
        ...
        if chat_id:
            try:
                ...
                logger.error(f"Failed to notify doctor {chat_id} for order {order.id}: {e}")
        ...
        if not chat_id:
            ...
            try:
                ...
                logger.error(f"Failed to notify manager {manager_id} for order {order.id}: {e}")
...
async def notify_couriers_about_order(bot, order: Order, session: AsyncSession):
    ...
    for courier in couriers:
        try:
            ...
            logger.error(f"Failed to notify courier {courier.telegram_id}: {e}")
    
    logger.info(f"Notified {notified_count}/{len(couriers)} couriers about order #{order.id}")
